...
const GitHubStarsButton = ({
  ...
  const [localIsScrolled, setLocalIsScrolled] = React.useState(false);
  const finalIsScrolled = 'isScrolled' in props ? isScrolled : localIsScrolled;

  React.useEffect(() => {
    if ('isScrolled' in props) return;
  ...
  return (
    <motion.a
      ...
      onClick={handleClick}
      className={cn(
        "flex items-center gap-2 text-sm bg-primary text-primary-foreground rounded-lg px-6 py-1 h-8.5 has-[>svg]:px-3 cursor-pointer whitespace-nowrap font-medium transition-colors disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-[18px] shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
        className,
      )}
      {...props}
    >
      ...
      <span className={cn("transition-all duration-300 overflow-hidden", finalIsScrolled ? "w-0 opacity-0" : "w-auto opacity-100")}>
        GitHub Stars
      </span>
      <div className="relative inline-flex size-[18px] shrink-0">
        ...
        <Star
          ...
          aria-hidden="true"
          style={{
            clipPath: `inset(${100 - (isCompleted ? fillPercentage : fillPercentage - 10)}% 0 0 0)`, /* Using fillPercentage as before */
          }}
        />
        <AnimatePresence>
          {displayParticles && (
